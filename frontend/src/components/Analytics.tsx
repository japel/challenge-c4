...
const Analytics: React.FC = () => {
  ...
  const { searchCount, cacheCount, averageResponseTime, averageResponseTimeCached, topKeywords } = data.data;
  
  return (
    <Stack gap="xl">
      <Title order={2} mb="md">Search Analytics</Title>
          ...
          <Card withBorder shadow="sm" p="lg" radius="md" h="100%">
            <Stack gap="md">
              ...
              <Group justify="space-between">
                <Stack gap={0}>
                  <Text size="lg" fw={500}>Total Searches</Text>
                  <Text c="dimmed">Number of searches performed</Text>
                </Stack>
                <Text size="xl" fw={700} c="blue">{searchCount}</Text>
              </Group>

              <Group justify="space-between">
                <Stack gap={0}>
                  <Text size="lg" fw={500}>Total Cache hits</Text>
                  <Text c="dimmed">Number of cache hits</Text>
                </Stack>
                <Text size="xl" fw={700} c="blue">{cacheCount}</Text>
              </Group>
              
              <Group justify="space-between">
                <Stack gap={0}>
                  <Text size="lg" fw={500}>Average Response Time (no cache)</Text>
                  <Text c="dimmed">Time to process search requests</Text>
                </Stack>
                <Text size="xl" fw={700} c="blue">{averageResponseTime.toFixed(2)}ms</Text>
              </Group>
              <Group justify="space-between">
                <Stack gap={0}>
                  <Text size="lg" fw={500}>Average Response Time (with cache)</Text>
                  <Text c="dimmed">Including cached responses</Text>
                </Stack>
                <Text size="xl" fw={700} c="blue">{averageResponseTimeCached.toFixed(2)}ms</Text>
              </Group>
            </Stack>
          </Card>
        </Grid.Col>
                      ...
                      <Group key={item.keyword} justify="space-between" wrap="nowrap">
                        <Box style={{ width: '70%' }}>
                          <Group justify="space-between" wrap="nowrap">
                            ...
                            <Text size="sm" c="dimmed">{item.count} searches</Text>
                          </Group>
                          <div
                            style={{
                              height: '8px',
                        ...
                        <RingProgress
                          ...
                          label={
                            <Center>
                              <Text size="xs" fw={700}>{percentage}%</Text>
                            </Center>
                          }
                        />
                      </Group>
